...
class TestTsort(unittest.TestCase):
    ...
    def test_05(self):
        ...
        with self.assertRaises(ValueError) as error_message:
            tsort(input)
            self.assertEqual(error_message, 'input contains an odd number of tokens')

    def test_06(self):
        input = []
        with self.assertRaises(ValueError) as error_message:
            tsort(input)
            self.assertEqual(error_message, 'input contains no edges')

    def test_07(self):
        input = ['1', '2', '2', '1', '3', '4']
        with self.assertRaises(ValueError) as error_message:
            tsort(input)
            self.assertEqual(error_message, 'input contains a cycle')
