...
class TestHeap(unittest.TestCase):
    ...
    def test_12_peek(self):
        ...
        test_heap.enqueue(900)
        self.assertEqual(test_heap.peek(), 900)

    def test_13_peek_empty(self):
        test_heap = MaxHeap()
        self.assertEqual(test_heap.peek(), None)

    def test_14_enqueue_full(self):
        test_heap = MaxHeap(6)
        test_heap.build_heap([2, 9, 7, 6, 5, 8])
        self.assertFalse(test_heap.enqueue(5))

    def test_15_dequeue_empty(self):
        test_heap = MaxHeap(0)
        self.assertEqual(test_heap.dequeue(), None)

    def test_16_heapsort_presorted(self):
        test_heap = MaxHeap(1000)
        sorted_list = [1, 2, 3, 4, 5]
        test_heap.heap_sort_ascending(sorted_list)
